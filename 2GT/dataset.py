...
def load_geom_gcn_dataset(name):
    ...
    adj = sp.coo_matrix(adj)#构造稀疏矩阵
    adj = adj + sp.eye(adj.shape[0]) #加上单位矩阵
    adj = adj.tocoo().astype(np.float32)#转化成float型
    features = np.array(
        [features for _, features in sorted(G.nodes(data='features'), key=lambda x: x[0])])#每个节点的特征被存储到数组中
    labels = np.array(
        [label for _, label in sorted(G.nodes(data='label'), key=lambda x: x[0])])#每个节点的标签被存储到数组中
    print(features.shape)

    def preprocess_features(feat): #特征预处理
        """Row-normalize feature matrix and convert to tuple representation"""
        #行归一化矩阵并转化成元组表示
        rowsum = np.array(feat.sum(1)) #行之和
        rowsum = (rowsum == 0) * 1 + rowsum
        r_inv = np.power(rowsum, -1).flatten() 
        r_inv[np.isinf(r_inv)] = 0.#将元素为无穷大的赋予0
        r_mat_inv = sp.diags(r_inv)#转化为对角矩阵
        feat = r_mat_inv.dot(feat)#点积
        return feat
    features = preprocess_features(features)#归一化处理

    edge_index = torch.from_numpy(
        np.vstack((adj.row, adj.col)).astype(np.int64))#
    node_feat = torch.FloatTensor(features)#节点特征
    labels = torch.LongTensor(labels)#标签
    num_nodes = node_feat.shape[0]#节点数量
    print(f"Num nodes: {num_nodes}")
