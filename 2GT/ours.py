...
class SGFormer1(nn.Module):
    ...
    def forward(self,G,node_feat, edge_attr_tensor,data):
        ...
        x3=self.trans_conv2(x)
        if self.use_graph:
            x4=self.gnn(G, node_feat, edge_attr_tensor)#加入MPNN
            if self.aggregate=='add':
                x=self.graph_weight*x4+(1-self.graph_weight)*x3
            else:
                x=torch.cat((x3,x4),dim=1)
        else:
            x=x3
        ...
        x5=self.trans_conv3(x)
        if self.use_graph:
            x6=self.gnn(G, node_feat, edge_attr_tensor)#加入MPNN
            if self.aggregate=='add':
                x=self.graph_weight*x6+(1-self.graph_weight)*x5
            else:
    ...
    def reset_parameters(self):
        ...
        #self.trans_conv4.reset_parameters()
        #self.trans_conv5.reset_parameters()
        #self.bn.reset_parameters()
        #self.trans_conv7.reset_parameters()
        if self.use_graph:
            self.gnn.reset_parameters()